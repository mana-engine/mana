...
// --- Device-agnostic proof (issue #221, #230): the same `move` action, driven by a GAMEPAD
// LEFT STICK (injected via #219's `pad_axes`, ADR 0028 amendment) or the GAMEPAD D-PAD
// (injected via `pad_buttons`, ADR 0040 §4 amendment) instead of the keyboard, must steer
// pac IDENTICALLY. Step 11 mirrors the keyboard open up-turn (08) via the d-pad; step 12
// mirrors it via `left_y=-0.9`; step 13 mirrors the keyboard up-then-down round trip (10)
// via `left_y` -0.9 then +0.9. All assert the exact positions their keyboard twin does, so
// "keyboard, stick, and d-pad all drive the same action to the same result" (ADR 0040 §8)
// is literally proven, not asserted. --

test "pacman scenario [d-pad up]: a d-pad-up press turns pac onto column 8 IDENTICALLY to keyboard 'up' and the left stick — device-agnostic proof (issue #230, mirrors 08/12)" {
    try requireLua();
    try expectScenarioPasses(std.testing.allocator, std.testing.io, pacman_paths, "games/pacman/scenarios/11_gamepad_dpad_turn.zon");
}

test "pacman scenario: two independent replays of the d-pad-up staircase agree bit-for-bit (issue #230)" {
    try requireLua();
    try expectDeterministic(std.testing.allocator, std.testing.io, pacman_paths, "games/pacman/scenarios/11_gamepad_dpad_turn.zon");
}
