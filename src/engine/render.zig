...
//!
//! The flat `Appearance`-quad path (`project`) lives here; the textured, animated
//! sprite-quad path (`projectSprites`) lives in the sibling `render_sprite.zig` (issue
//! #151 — split out once it grew its own depth-sort and test suite) and is re-exported
//! below so the public API (`render.projectSprites`) is unchanged. Both share the
//! projection math below (`View`, `Projection`, `projectPoint`, `screenFacing`,
//! `pxPerWorldUnit`).
...
pub fn pxPerWorldUnit(proj: Projection) f32 {
    return switch (proj) {
...
/// the screen pixel that world `(0,0,0)` maps to (typically the viewport centre). Pub:
/// shared with `render_sprite.zig`'s `projectSprites`. Greater `depth` = nearer/front
/// (drawn later, lands on top). Pure and total.
pub fn projectPoint(proj: Projection, pos: core.Vec3, origin: core.Vec2) struct { screen: core.Vec2, depth: f32 } {
    return switch (proj) {
        .orthographic => |o| .{
...
/// screen +x ⇒ right, −x ⇒ left, +y ⇒ down, −y ⇒ up (ties resolve horizontal). Pub:
/// shared with `render_sprite.zig`'s `projectSprites`. Pure.
pub fn screenFacing(proj: Projection, heading: core.Vec2, origin: core.Vec2) ?data.msf.Facing {
...
/// The textured, animated sprite-quad projection (issue #113 phase 2b; ADR 0031 §4, ADR
/// 0033) — split into `render_sprite.zig` (issue #151) and re-exported here so the
/// public API is unchanged. See that file for the full doc.
pub const projectSprites = @import("render_sprite.zig").projectSprites;
