//! `ui`'s parse/layout/hit-test core (ADR 0034 §§1-4; issue #132 Phase 1, split out of
//! `ui.zig` per issue #151): the widget vocabulary (`Kind`/`Layout`/`Direction`/
//! `Anchor`), the `Widget`/`Screen` tree ZON `parse`s into, `layout` (pure geometry into
//! `Placed` rects), and `hitTest`/`consumesPointer`/`focusOrder` (point/tree queries over
//! a `layout` result). Kept separate from `focus.zig`'s stateful `Focus` nav so both
//! sibling files can import these shared leaf types without either importing the other
//! back — `ui.zig` re-exports everything here so the public API (`ui.Widget`,
//! `ui.layout`, `ui.hitTest`, …) is unchanged.
...
test "ui.types: parse round-trips a nested widget tree with defaults" {
    const src =
        \\.{
        \\    .name = "hud",
        \\    .root = .{
...
test "ui.types: parse reads the optional id field, defaulting to empty (ADR 0039 §2)" {
    const src =
        \\.{
        \\    .root = .{
        \\        .kind = .container,
...
test "ui.types: layout — anchor container positions sized children in its padded content" {
...
test "ui.types: layout — flex row splits leftover space among unsized children" {
...
test "ui.types: layout — flex column stacks children by height with gaps" {
    const children = [_]Widget{
...
test "ui.types: hitTest returns the topmost (deepest) widget under a point" {
    const children = [_]Widget{
...
test "ui.types: hitTest edge exclusivity — abutting rects never both claim the boundary" {
    const children = [_]Widget{
        .{ .kind = .panel, .width = 50 },
        .{ .kind = .panel, .width = 50 },
    };
...
test "ui.types: consumesPointer is true over any widget, false off-screen" {
    const screen: Screen = .{ .root = .{ .kind = .panel } };
    const placed = try layout(testing.allocator, &screen, .{ .x = 0, .y = 0, .w = 100, .h = 100 });
    defer testing.allocator.free(placed);
...
test "ui.types: focusOrder collects only focusable widgets, in paint order" {
    const children = [_]Widget{
